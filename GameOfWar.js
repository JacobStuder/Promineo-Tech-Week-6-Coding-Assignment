...
class Card { // class to create cards
...
class Deck { //class to create a deck of 52 cards put into an array with a loop to randomize the elements in that array
    constructor() { // and another loop to distribute the first element to one array and the next element to another array.
        this.cards = [];
    };
...
class Player { // class to hold player names, with an array for player hand, and to start player score at zero
...
class Play { // class to initiate a new deck with a loop to increment rounds with randomized first element of playerHand array
    start() { // and a if/else statement to increment player score by round
        player1.newRound();
        player2.newRound();
...
The score remains ${player1.playerName} ${player1.playerScore} - ${player2.playerName} ${player2.playerScore}`);
    ...
    scoring() { // if/else statement to display final score
...
let newPlay = new Play(); // Instance to instantiate the start application method
newPlay.start();
